...
# 0-based left closed and right open interval\n---\n"""%(self.transcriptid,self.transcriptname,self.transcript_type,self.chrom,self.strand,self.exons,self.intron,self.leftmost,self.rightmost,self.cds)
    ...
    def merge_exons(self):
        ...
        self.exons = seqio.merge_region(self.exons)[0]
        self.merged = 1
        return 0

    def add_exon(self,start,end): # add to a transcript instance 
        if start >= end: 
            #  这里其实有个问题，添加 start 和 end 时，一定要保证 start  < end, 且这里是 0-based [a,b) , 若gtf 和 gff 类的1-based 坐标体系，strand 为 - 时，start  > end ，一定要交换后，再对小的坐标 -1
            # 有的文件中，会存在= 的现象，往往是ncRNA，即没有coding 区，导致cds start 和 cds end 一致
            return 0
        if self.parsed:
...
class Gffregion(object):
    def __init__(self,fmt='gff',gidattr="ID",gattr="Name",tidattr="ID",tattr="ID",pattr="Parent=",pidattr="protein_id=",codonanno_start="CDS",codonanno_stop="CDS",ganno="gene,",tanno="rRNA,mRNA,tRNA",eanno="exon,CDS,UTR"):
        ...
        self.ganno   = dict.fromkeys(ganno.split(","),None)
        self.tanno   = dict.fromkeys(tanno.split(","),None)
        self.eanno   = dict.fromkeys(eanno.split(","),None)
        self.pattr   = Key_obj(pattr)
        self.hgenes  = {}
        self.htranscripts = {}
        
    def __get_codon(self,start,end,gid,tid,chrom,strand,based=1,startanno = 1):
        ...
        try:
            ...
            return 0
        start = start-based
        tmpgene = self.hgenes[gid]
        assert tid in tmpgene.transcripts
        tmptranscript = tmpgene.get_transcript(tid)
    ...
    def gff2exons(self,gfffn):
        ...
        for line in f:
            ...
            if region_type in self.ganno:
                gid   = self.gidattr.getvalue(commnet)
                gname = self.gattr.getvalue(commnet)
                tmpgene = self.hgenes.setdefault(gid,Gene(gid,gname,gene_type=region_type))
            elif region_type in self.tanno:
            ...
            if region_type in self.codonanno_start:
                tid   = self.pattr.getvalue(commnet) # tid
                gid   = self.htranscripts[tid]
                self.__get_codon(start,end,gid,tid,chrom,strand)
            if region_type in self.codonanno_stop:
                tid   = self.pattr.getvalue(commnet)
                gid   = self.htranscripts[tid]
                self.__get_codon(start,end,gid,tid,chrom,strand,startanno=0)
        f.close()
        for gid in self.hgenes:
            self.hgenes[gid].parse_gene()  ## parsed ok
        return 0
    def __str__(self):
        strout = ""
        for gid in self.hgenes:
            strout += str(self.hgenes[gid])
            break
        strout += "... ..."
        return strout

# use for `gtf` file foramt
