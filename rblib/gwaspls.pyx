...
def resampling1(wvector,numselect):
    ...
    wvector = np.float64(wvector)
    pro = wvector/np.sum(wvector)
    procum = np.cumsum(pro)
    #sys.stderr.write("[INFO]procum done\n")
    ...
    for i in range(numselect):
        ...
        temp = randnum < procum
        temp = list(temp)
        stmp = temp.index(True)
        Index.append(stmp)
        print(stmp,np.sum(randnum > procum)) # instead method, more faster
    """
    #for i in pbar.progressbar(range(numselect)):
    for i in range(numselect):
        #sys.stderr.write("[INFO]select: %d\n"%(i+1))
        if (i+1) % 50000 == 0:
            sys.stderr.write("[INFO] resample times: %d"%(i+1))
        Index.append(np.sum(np.random.rand() > procum))
    return Index
...
#        print prosum
        ...
        procum = np.cumsum(pro)
        return pro,procum
    #for i in pbar.progressbar(range(numselect-1)):
    for i in range(numselect-1):
        #if (i+1) % 50000 == 0:
        #    sys.stderr.write("[INFO] resample times: %d"%(i+1))
        ind = np.sum(np.random.rand() > procum)
        Index.append(ind)
        pro,procum = weightas0(pro,ind,procum)
    ind = np.sum(np.random.rand() > procum); Index.append(ind)
    return Index
...
def pca(X_SNPs,nvs_output=10,norm=1):
    """
    ...
    sys.stderr.write("[INFO] total error: %5.g\n"%totalerr)
    i = 0
    while i<nvs_output:
        maxerror = 1
        iter = 1
        while(i+1):
            if maxerror < eps:
                ...
                t0 = tvec
                if iter % 10 == 0: sys.stderr.write("[INFO]: LV %d, Error after %d iterations: %5.g\n"%(i+1,iter,maxerror))
                iter += 1
...
def pls_validataion_risk(X0,Y0,Xmean,Xstd,Ymean,Ystd,Xtest,Ytest,nlvs,method='quantificat',retpred=0):
    ...
    centring(Xtest_scale,Xmean=Xmean)
    normalize(Xtest_scale,Xstd=Xstd)

    ypredict_raw = np.dot(Xtest_scale,betatmp)
    ypredict = ypredict_raw * Ystd + Ymean

    if method == "quantificat":
        risk = np.sqrt(np.sum(np.asarray(Ytest - ypredict)**2)/nx)
    else:
        risk = np.sum(np.sign(ypredict_raw) == np.sign((Ytest - Ymean))) / float(nx)
    if retpred:
